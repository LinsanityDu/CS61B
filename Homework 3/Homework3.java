...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
	...
	//  int dupValue=ints[0];
	  int[] newArray;
	  newArray= new int[ints.length];
	  while (oldArrayIndex<ints.length-1) {
		  if (ints[oldArrayIndex] != ints[oldArrayIndex+1]) {
			  newArray[newArrayIndex]=ints[oldArrayIndex];
			  newArrayIndex++;
		  }
		  oldArrayIndex++;
	  }
	  ...
	  for (int i=newArrayIndex+1; i<newArray.length; i++) {
		  newArray[i]=-1;
	  }
